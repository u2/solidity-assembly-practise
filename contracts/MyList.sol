contract MyList {
  ...
  function getLength(address _a, uint key) constant returns(uint) {
    return data[key].length;
  }

  function getReturnSize(address _a, uint key) constant returns(uint) {
  ...
  function getAll(address _a, uint key) constant returns(uint[]) {
    return data[key];
  }
